...
def run():
	...
	if DRAW_PIC:
	    draw_plot(x_list,cpu_y,'CPU','Time(s)','%',m)
	    draw_plot(x_list,mem_y,'MEM','Time(s)','%',m)
	    draw_plot(x_list,res_y,'RES','Time(s)','%sB'%(res_unit),m)
	...
	if DRAW_PIC:
	    draw_plot(x_list,mem_y,'MEM','Time(s)','%','MEM')
	    draw_plot(x_list,gpu_y,'GPU','Time(s)','%','GPU')
    if NETWORK_FLAG:
	x_list = []
	y_list = []
