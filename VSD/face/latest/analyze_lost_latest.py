...
IMAGE_HEIGHT = 1080
THRESHOLD = 0.1
TOPN = 0 #0 means use ALARM_THRESHOLD
#SECONDS = 50
ALARM_THRESHOLD = 0.6
...
def get_capture_rate(vname):
    ...
    tlist = result_to_list("/home/dell/python/sun/deepface/vsd/latest/test_result/result.txt")
    ioucnt = 0
    for gitem in glist:
       gframeid = gitem[0]
       gid = gitem[1]
...
def calculate_presition(t_frameid, gid, vname): #输入为真实frameid，人id 
    ...
    flist = result_to_list("/home/dell/python/sun/deepface/vsd/latest/test_result/result.txt")
    ...
    for num in reversed(tmplist): #倒序往前推, num是存起来的vsd frameid
	  ...
	  k_cnt = 0
		 ...
		 if TOPN == 0:
			 ...
			 print "confidence:%s"%str(fitem[i*3+5])
