...
conf = dict(
        ...
        endpoints = dict(
            single = "http://192.168.2.19:6601/rec/image",
            batch = "http://192.168.2.19:6601/rec/image" + "/batch",
        ),
        batchNum = 8,
...
class QueryThread(threading.Thread):
    def __init__(self, root_path,imageFile,objectType,function):
        threading.Thread.__init__(self)
        self.setDaemon(True)
        self.root_path = root_path
        self.functions = eval(function) #conf["functions"]
        self.objectType = eval(objectType)
    ...
    def SingleReq(self, query):
        ...
        post_dict['Image'] = {"Data": query_dict,"UserObject":[{"Type": self.objectType}]}
    ...
    def BatchReq(self, batchQuery):
        ...
        for query in batchQuery:
            ...
            if conf['uriType'] == 0:
		        ...
		        query_dict = {'URI': query}
            batch_image.append({"Data": query_dict,"UserObject": [{"Type": self.objectType}]})
...
#do load test
def LoadTest(imageFile,objectType,function):
    testThreads = []
    for i in range(conf['threadNum']) :
        thrd = QueryThread(conf['mountPath'] + conf['photoPath'],imageFile,objectType,function) 
...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option("-o","--objectType",action="store",dest="objectType",help="ObjectType 1:face,2-vehicle,3-nomotor,4-pedestrian")
    parser.add_option("-i","--imageFile",action="store",dest="imageFile",help="image url file list")
    parser.add_option("-f","--function",action="store",dest="function",help="Functions [200,201,202,203,204,205]")
    (options, args) = parser.parse_args()
    LoadTest(options.imageFile,options.objectType,options.function)
