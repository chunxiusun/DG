...
conf = dict(
        ...
        address = "http://192.168.2.164:6501/rec/image",     # service address
...
class QueryThread(threading.Thread):
	...
	#matrix return code
        ...
        for result in results:
            ...
            if result.has_key("Faces") == False:
                #print "ERROR Faces"
                #print result
                #stat["errorCount"] += 1
                #stat["errorTime"] += elapse
                return "" 
            car_num  = len(result['Faces'])
        ...
        while True:
            ...
            with open(filename, "a") as f:
				...
				if r.has_key("Result") == False:
				    ...
				    continue
				for face in r['Result']['Faces']:
				    if "204" not in conf["functions"]:
					#print "no function 21"
					continue
				    if face.has_key("Features") == False:
					logger.error("r does not has key Features")
					continue
                                    feature = str(face['Features'])
				...
				for result in r['Results']:
                                    if "204" not in conf["functions"]:
                                        continue
                                    if result.has_key("Faces") == False:
                                        logger.error("result does not has key Faces")
                                        continue
                                    for face in result["Faces"]:
                                        if face.has_key("Features") == False:
                                            logger.error("face does not has key Features")
				            continue
					feature = str(face['Features'])
                                        feature = feature.replace('\n','')
                                        if feature == "":
