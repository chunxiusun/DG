...
conf = dict(
        ...
        #endpoints = dict(
            #single = "http://%s:%s/rec/image"%(matrixIp,matrixPort),
            #batch = "http://%s:%s/rec/image/batch"%(matrixIp,matrixPort),
        #),
        batchNum = 8,
...
class QueryThread(threading.Thread):
    ...
    def Search(self, query):
        ...
        if conf['mode'] == 0:
            ...
            m_url = "http://%s:%s/rec/image"%(conf["matrixIp"],conf["matrixPort"])
        ## batch
        elif conf['mode'] == 1:
            post_data = self.BatchReq(query)
            #m_url = conf['endpoints']['batch']
            m_url = "http://%s:%s/rec/image/batch"%(conf["matrixIp"],conf["matrixPort"])
...
def getOptions():
    try:
        options,args = getopt.getopt(sys.argv[1:],"h",["help","ip=","port=","mode=","batchsize=","threadnum=","imagefile="])
    ...
    for name,value in options:
        ...
        if name in ("--threadnum"):
            conf["threadNum"] = eval(value)
        if name in ("--imagefile"):
            conf["img_url_file"] = value
