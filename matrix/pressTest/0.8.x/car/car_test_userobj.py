...
conf = dict(
        ...
        endpoints = dict(
            single = "http://192.168.2.234:6501/rec/image",
            batch = "http://192.168.2.234:6501/rec/image" + "/batch",
        ),
        batchNum = 8,
        functions = [101,102,103,104,105,106,107,108,109,300,301,400],
        objecttype = 2, # 1:face,2-vehicle,3-nomotor,4-pedestrian
...
class QueryThread(threading.Thread):
    ...
    def SingleReq(self, query):
        ...
        post_dict['Image'] = {"Data": query_dict,"UserObject":[{"Type": conf['objecttype']}]}
    ...
    def BatchReq(self, batchQuery):
		...
		query_dict = {'URI': query}
            batch_image.append({"Data": query_dict,"UserObject":[{"Type": conf['objecttype']}]})
	...
	#matrix return code
		...
		#print post_data
		logger.info('result does not has key Vehicles')
                continue
            ...
            #Pedestrian
            if not result.has_key("Pedestrian"):
                #print post_data
                logger.info('result does not has key Pedestrian')
                continue
            ...
            for (i, bound) in enumerate(conf['pedestrianNumBound']) :
                if pedestrian_num < bound :
                    ...
                    break
            if car_num > conf['pedestrianNumBound'][-1]:
...
class StatThread(threading.Thread) :
    ...
    def StatInfo(self, requestCount, timeoutCount, requestTime,
            ...
            statusCodeDist, carNumDist, pedestrianNumDist,carCount,pedestrianCount, plateCount, colorCount
            ) :
        elapse *= 1.0
        ...
        statInfo = "### Search time:%d, ErrorCount:%d, HttpErrorCount:%d,\n"\
                   ...
                   "### car per photo:%.2f pedestrian per photo:%.2f,\n"\
                   "### elapse:%ds\n" % (
        ...
        for i in range(
            ...
            statInfo += "%-22s %-22s %-0s %-5s %-25s %-25s\n" % (str1, str5, str6, str2, str3, str4)
    ...
    def run(self) :
        while conf["onePass"] != 2 :
            ...
            if conf["verbose"] :
                ...
                statInfo += self.StatInfo(self.requestCount, 
                        ...
                        self.httpErrorTime, self.statusCodeDist, self.carNumDist,self.pedestrianNumDist, 
                        self.carCount, self.pedestrianCount, self.plateCount, self.colorCount)
                statInfo += "=-" * 38 + "\n"
	    ...
	    statInfo1 = "#" * 26 + str(datetime.datetime.now()) + "#" * 26 + "\n"
            statInfoAll = self.StatInfo(
                    ...
                    stat["statusCodeDistAll"], stat["carNumDistAll"],stat["pedestrianNumDistAll"],
                    stat["carCountAll"], stat["pedestrianCountAll"], stat["plateCountAll"], stat["colorCountAll"])
