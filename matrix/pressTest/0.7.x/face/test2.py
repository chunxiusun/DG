...
conf = dict(
        ...
        address = "http://127.0.0.1:6601/rec/image",     # service address
        batch_num = 8,                 # batch num
        thread_num = 30,                 # thread num
        photo_path = "/srv/ftp/fengziyong/Image",           # image path
...
class QueryThread(threading.Thread):
	...
	print "sun+++++++++++++++++++++++++++++++++++++"
	print returnCode
	print resp_dict['Context']['Message']
        if returnCode != 200:
	    print "sun=================================="
	    print returnCode
	    print resp_dict['Context']['Message']
				...
				for i in range(0,len(r['Results'])):
				    for j in range(0,len(r['Results'][i]['Pedestrian'])):
					    ...
					    if r['Results'][i]['Pedestrian'][j]['Face'].has_key("Features") == False:
                                                logger.error("r does not has key Features")
                                            feature = str(r['Results'][i]['Pedestrian'][j]['Face']['Features'])
                                            feature = feature.replace('\n','')
                                            if feature == "":
