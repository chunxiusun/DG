...
conf = dict(
        #address = "http://192.168.2.16:6111/rec/image",     # service address
        #address = "http://192.168.2.16:6121/rec/image",     # service address
        address = "http://192.168.2.19:9805/rec/image",     # service address
        batch_num = 8,                 # batch num
        thread_num = 100,                 # thread num
        photo_path = "/home/dell/data/matrix/test_face/Image/",           # image path
        #input_path = "input",           # file input path
        #input_path = "input_rengong",           # file exception 
        #input_path = "input_2046",           # file performace
        #input_path = "input_single",           # file ziyong
        input_path = "input_10w",           # file ziyong
        #input_path = "input_multi2",           # file baike
        mode = 0,                       # 0 means single, 1 means batch
        read_method = 1,                # 0 means read from files, 1 means read from folder
        uri_type = 3,                   # 0 means local path, 1 means http path, 2 means encode to base64, 3 means image url file
        svr_type = 2,                   # 1:car 2:face 3:all 0:default(face)
        functions = [2,20,22,23],
	img_url_file = 'face-img_http.txt',
        #uri_header = 'http://192.168.2.16:3003/', # for /data
        uri_header = 'http://192.168.2.16:3004/', # for /mmm/xueboliu/MsCelebV1-Faces-Cropped/
        #uri_header = 'http://192.168.2.16:3004/chenzhen/test_feature', # for /mmm/xueboliu/MsCelebV1-Faces-Cropped/
        file_header = '',
        #file_header = '/home/dell/xiaodong/data/',
...
SESSION_ID = 0
timeBound = (200, 300, 400, 500, 600, 800, 1000, 1500, 2000, 5000, 20000)
...
class QueryThread(threading.Thread):
    ...
    def UriQuery(self, query):
        ...
        if conf['uri_type'] == 0:
            #print "in###########"
            #print  conf['file_header']
            #print query
            #query_dict = {'URI': 'file:' + query}
            query_dict = {'URI': 'file:/' + conf['file_header'] + "/" + query}
    ...
    def Search(self, query):
        ...
        #print "############post data########"
        #print post_data
        tmp_data = eval(post_data)
        if tmp_data.has_key('Images'):
            logger.info('URI:' + tmp_data['Images'][0]['Data']['URI'])
	    logger.info('post_data:' + post_data)
            #print tmp_data['Images'][0]['Data']['URI']

        begin = time.time()
        try:
            resp = requests.post(m_url, data=post_data, timeout=conf['timeout']*1.0/1000)
        #except:
        ...
        try:
            ...
            print "json.loads ERROR Exception"
	...
	#matrix return code
        ...
        results = []
        if conf['mode'] == 0:
	    if resp_dict.has_key("Result") == False:
                logger.error("Error: resp_dict does not has key Result")
            else:
                result = resp_dict['Result']
                results.append(result)
        elif conf['mode'] == 1:
	    if resp_dict.has_key("Results") == False:
                logger.error("Error: resp_dict does not has key Results")
            else:    
        ...
        for result in results:
            ...
            if result.has_key("Pedestrian") == False:
                print "ERROR Pedestrian"
                #stat["errorCount"] += 1
                #stat["errorTime"] += elapse
                return "" 
            car_num  = len(result['Pedestrian'])
                ...
                while True:
                    ...
                    if conf['mode'] == 0:
                        if len(batch_list) >= 1:
                            ...
                            if r:
				#sun
				if r.has_key("Result") == False:
				    logger.error("r does not has key Result")
				    #continue
				for i in range(0,len(r['Result']['Pedestrian'])):
                                    if len(r['Result']['Pedestrian']) >0 and r['Result']['Pedestrian'][i].has_key('Face'):
				        if "21" not in conf["functions"]:
					    #print "no function 21"
					    continue
				    if r['Result']['Pedestrian'][i]['Face'].has_key("Features") == False:
					logger.error("r does not has key Features")
					#continue
                                    feature = str(r['Result']['Pedestrian'][i]['Face']['Features'])
                        ...
                        if (self.stopped == True and len(batch_list) > 0 ) or len(batch_list) >= batch_num: 
                            ...
                            if r:
                                #for i in range(0, len(batch_list)):
				#sun
				if r.has_key("Results") == False:
				    logger.error("r does not has key Results")
				    #continue
				for i in range(0,len(r['Results'])):
				    for j in range(0,len(r['Results'][i]['Pedestrian'])):
                                        if len(r['Results'][i]['Pedestrian']) > 0 and r['Results'][i]['Pedestrian'][j].has_key('Face'):
                                            if "21" not in conf["functions"]:
						#print "no function 21"
					        continue
					    if r['Results'][i]['Pedestrian'][j]['Face'].has_key("Features") == False:
						logger.error("r does not has key Features")
						continue
...
class StatThread(threading.Thread) :
	...
	log_name_1 = "./log/stat.restful_all.log." + str(conf["requestID"])
	...
	self.logger_1.info("thread_num: %d", conf["thread_num"])
    ...
    def StatInfo(self, requestCount, timeoutCount, requestTime,
        ...
        statInfo = "### Search time:%d, ErrorCount:%d, HttpErrorCount:%d,\n"\
                   ...
                   "### Timecost per 1W::%.2fs,\n"\
                   "### colorCount:%d,%.4f plateCount:%d,%.4f faceCount:%d,\n"\
                   "### face per photo:%.2f,\n"\
                   "### elapse:%ds\n" % (
        ...
        carNumDistStr = [];
        carNumDistStr.append("Face Num distro:")
    ...
    def run(self) :
	    ...
	    self.logger_1.info(statInfo1 + statInfoAll)
            ...
            #self.logger_1.info(statInfoAll)
...
def LoadTest() :
    ...
    status, result = commands.getstatusoutput("find " + conf['feature_file'] + " -type f -size 0c | xargs -n 1 rm -f")


    while conf["onePass"] != 1:
